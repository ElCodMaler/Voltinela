...
function Categorias({info, productos}) {
  ...
  const renderCategorias = () => {
    return (
      <>
        <div className='container mx-auto px-4 sm:mx-10 md:mx-16 xl:mx-28 2xl:mx-36' id='categorias' style={{width:'-webkit-fill-available'}}>
          <Card id={categorias[0]} className="carta w-full py-12 bg-cover bg-center bg-[url('/respaldo-playa.jpg')] bg-gray-400 bg-blend-multiply shadow-xl md:h-48 lg:h-72 xl:h-48 2xl:h-60" onClick={() => handleSelectedCat(categorias[0])}>
            <h5 className="text-center text-2xl font-bold tracking-tight text-white md:text-3xl lg:text-5xl xl:text-4xl 2xl:text-5xl">
              {categorias[0]}
            </h5>
          </Card>
          <br />
          <div className='grid gab-2 grid-cols-2 space-x-3 md:h-48 lg:h-72 xl:h-48 2xl:h-60'>
    ...
    <>
      <section className='flex flex-col items-center w-full pt-5'>
        <div className='titulo container mx-auto w-max lg:py-4 xl:py-1 2xl:py-2'>
           <h3 className='text-lg text-white text-center font-bold px-4 py-1 rounded-full shadow-xl md:text-3xl md:py-2 lg:text-4xl lg:py-3 xl:text-3xl xl:px-8 2xl:text-4xl 2xl:py-2' style={{backgroundColor:'#81cc00'}}>{info.categorias}</h3>
        </div>
        <br />
        {renderCategorias()}
      </section>
